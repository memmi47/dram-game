...
| H-002 | 1회 플레이 시간 | 30~40분 | 동의 | **승인(복귀 확정)** | 30~40분 승인 → 멀티 전환 논의 중 폐기 → 멀티 철회 후 **원래 값 30~40분으로 복귀**. 경위는 `product-direction-v0.6.md` §5 |
...
| H-005 | 우선 플랫폼 | 외부 의존성 없는 단일 HTML 웹 | 동의 | 승인 | 기존 원칙 유지 — 싱글플레이어 확정(H-006)으로 계속 성립 |
| H-006 | 멀티플레이어 여부 | 싱글 유지, 멀티/M&A는 별도 프로젝트로 분리 | 동의 | 승인 | **싱글플레이어(오토마) 확정.** 멀티는 논의만 되고 코드 미구현 — 롤백 없음. `product-direction-v0.6.md` §2 |
| H-007 | 제품 명칭 범위 | (미제안) | 명칭 일반화 지지 | 승인 | **"메모리"로 일반화 — 명칭·프레이밍만.** NAND 등 제품군 실제 확장은 보류. H-002 확정으로 코드 표기 교체 **착수 가능** |
...
| ID | 결정 항목 | Codex 권장안 | Claude 의견 | 상태 | Human 결정 |
|---|---|---|---|---|---|
| BM-001 | Brass·Kanban EV·Silicon Valley·Acquire BM 반영 범위 | 기존 1인 게임의 표현·상태 레이어로 선택 통합. 전체 규칙 결합은 별도 2~4인 모드 후보로 분리 | 동의 — 전체 결합 반대, 선택 이식 지지 | **승인** | 싱글 확정(H-006) 기준으로 7축 재판정 완료 — `product-direction-v0.6.md` §4. 네트워크·M&A·턴순서는 **범위 밖**으로 확정 |
| BM-002 | 첫 BM 수직 슬라이스 | 5개 부서 Fab Operations Board + 프로젝트 타일 뒤집기 + 기존 인과 카드 연결 | 동의 | **승인(착수 가능)** | H-002=30~40분 확정 → 턴당 60초 예산 기준으로 설계. Codex 성공 기준(분기 중앙값 60초 이하)과 일치 |
| BM-003 | HBM 리듬게임 | 기본 루프 반려, 완료 연출로만 | 동의(손기술이 사후 설명성 훼손) | **반려 확정** | 양 에이전트 일치. 기본 루프에서 제외 |
...
| X-005 v0.5 최종 승격(초안) | `Claude` | **동결** | `docs/design-v0.5.md` — 문서 전체 PM 확인은 없었으나 v0.6 방향 변경으로 동결본이 됨. 전체 승인 대신 "동결 상태 확인"으로 갈음. S1 보류 사유는 H-002 확정으로 해소 |
| A-004 가상 브랜드명 초안 | `Codex` | PM 검토 대기 | `docs/brand-name-proposals-v0.1.md` |
| BM-001~003 BM 선택 통합안 | `Codex` | 완료 | `docs/benchmark-integration-v0.1.md` — 원작 공식 규칙서 4종 직접 확인 |
| C-DIR-001 싱글 재확정·메모리 일반화 | `Claude` | 완료 | `docs/product-direction-v0.6.md` — PM 결정 H-006/H-007 반영 |

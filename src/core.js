...
function runQuarter(state, playerDecisions, rng) {
  ...
  // P0-E fix: supply growing faster than the era trend must push price DOWN, per the reaction
  // function (Sufficiency = Supply/Demand; higher Sufficiency -> lower price). capacityPressure
  // is positive when supply outgrows trend, so it must be subtracted from dlnP, not added — the
  // previous `+` let flooding supply raise price in 9/14 seeds. Era calibration (ERA_TILT,
  // this coefficient) is deliberately left untouched here; it drifts after this fix and is
  // re-measured in S2, not patched in the same commit as the sign correction.
